...
IRQ0    
        ; SET Y COORDINATES
        ; TODO: PUT IN A SUBROUTINE, USED IN EVERY IRQ HANDLER
        LDA #$01
        STA $FA
        LDA #$D0
        STA $FB
...
SETCLR1 LDA $FC
        ...
        ; MOVE ALL SPRITES IN X AXIS
        LDX #0 ; SPRITE COUNTER
MOVE    TXA ; SET NUMBER OF SPRITE TO MOVE TO ACCUMULATOR
        JSR MOVEX
        INX ; INCREASE X
        CPX #8 ; IF NOT MOVED ALL THE SPRITES,
        BNE MOVE ; CONTINUE MOVING THEM
...
MOVEX   ; A REGISTER = SPRITE NUMBER 0-7
        STA $FD ; HOLDS SPRITE NUMBER
        PHA ; PUSH A, X AND Y TO STACK TO BE A GOOD CITIZEN,
        TXA ; ALTOUGH A AND Y ARE NOT NECESSARY IN THIS EXAMPLE TO BE STORED,
        PHA
        TYA
        PHA
        LDA $FD ; AND GET A BACK FROM MEMORY
     
        ASL ; A * 2, BECAUSE X POINTERS ARE THE EVEN ONES FROM $D000
        TAX ; PUT POINTER OFFSET TO X REGISTER
        ...
        LDY #0 ; MSB TOGGLE MASK SHIFT COUNTER
...
TOGGLE  EOR SPR_MSBX ; TOGGLE MSB,
        STA SPR_MSBX ; AND STORE MSB

NOMSB   PLA ; GET Y,X AND A BACK FROM THE STACK,
        TAY
        PLA
        TAX
        PLA
        RTS ; AND END SUBROUTINE.
